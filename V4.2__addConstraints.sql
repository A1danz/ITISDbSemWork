alter table Sticker
add constraint check_sticker_name check (length(name) >= 4),
...
add constraint check_media_channel_name check (length(name) >= 4),
add constraint check_media_channel_desc check (length(description) > 0);

alter table Bot
add constraint check_bot_name (length(name) >= 4),
...
add constraint check_client_type_name check (length(name) >= 4)